...
- `extension/batch-upload.js` — 🔥 **實作全量頁籤商品掃描**：將 `chrome.scripting.executeScript` 多頁籤 API 掃描升級為**第一優先執行** (解決舊版 `sendMessage` 回傳 35 筆單頁 DOM 攔截備用方案的破口)，涵蓋 `live_all` (架上)、`reviewing` (審核中)、`unpublished` (未上架) 與 `violation` (違規/刪除) 頁籤。介面顯示格式改為 `38 筆已上架商品 (35 筆架上 + 3 筆審核中/其他)`。
...
- `extension/content.js` — 🔥 **實作金額門檻分期條件**：當商品金額 (`numericPrice`) >= NT$ 1,000 時，自動選擇信用卡分期為「是」並設為 24 期 Bubble 按鈕；當金額 < NT$ 1,000 時保持「否」（避免觸發蝦皮金額限制警示）。
- `extension/batch-upload-test.js` — **同步 DOM 測試斷言**：在單件測試中加入金額條件判定的信用卡分期 DOM 斷言檢查，確保測試腳本與實際上架邏輯 100% 同步。

### 修復：重複上架根因修復、由上而下順序優化與防審核沉浸冷卻 (026-03 & 030 spec)

- `extension/batch-upload.js` — 🔥 **修復連線斷開引發 Retry 導致重複上架 (026-03 spec)**：在發送點擊「儲存並上架」按鈕後，加入 2 秒 Tab 網址跳轉比對。若網址已跳轉至商品列表頁或離開 `/portal/product/new`，代表蝦皮後台已成功創建商品，強制認定為成功發布，防範連線中斷誤判失敗引發重試二次上架；新增每筆商品間 `6000ms ~ 12000ms` 的隨機動態沉浸冷卻 (030 spec)，避開蝦皮風控佇列。
- `extension/content.js` — 🔥 **由上而下欄位順序重構與擬真 Jitter 停頓 (026-03 & 030 spec)**：重構 `fillAll()` 的執行順序為符合蝦皮 UI 的由上而下結構 (`名稱` -> `類別` -> `品牌` -> `尺寸` -> `描述` -> `價格` -> `數量` -> `信用卡分期`)，並跳過「最低數量」與「重量」的賦值填寫；新增欄位間 `150ms ~ 350ms` 的隨機 Jitter 停頓，避開機器人快速填寫特徵。
- `docs/spec/026-03-plan-重複上架根因修復與由上而下填寫順序優化.md` — 新增 026-03 規格計畫文件。
- `docs/spec/030-plan-避開機器人審核特徵與人類沉浸間隔優化.md` — 新增 030 規格計畫文件。
