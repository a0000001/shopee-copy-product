...
- `extension/batch-upload.js` — 🔥 **實作全量頁籤商品掃描**：擴充 `scanProducts()` API 爬取邏輯，涵蓋 `live_all` (架上)、`banned` (審核中/違規)、`unlisted` (未上架) 與 `deleting` 頁籤。解決舊程式僅爬架上 35 筆漏掉 3 筆審核中商品導致重複刊登問題，介面顯示格式改為 `38 筆已上架商品 (35 筆架上 + 3 筆審核中/其他)`。
