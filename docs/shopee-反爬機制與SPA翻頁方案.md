...
**可用參數**:
- `page_size=100`（一次拉完，不用翻頁）
- `list_type=live_all`（只回已上架）
- `operation_sort_by=recommend_v4`
- `need_ads=false`
- `SPC_CDS=<cookie值>` & `SPC_CDS_VER=2`

**失敗原因**: Shopee 採用動態請求簽章（SPC_CDS 等 token + header fingerprint）。**在主世界（main world）執行的 injected script / console script** 無法偽造這些簽章，回 403 `code=1000100018`。

**isolated world（content script）** 能否成功仍未驗證（擴充功能尚未在 DevTools MCP Chrome 中載入）。

## 最佳方案優先順序

### 方案 A：content script isolated world API（待驗證）
- 由 `content.js` 的 `extractSellerProductList()` 發起 fetch
- 因執行在 isolated world，或許可以繞過 Shopee 對 main world 的偵測
- 若成功則回退到方案 B

### 方案 B：SPA 點擊翻頁（confirmed working ✅）
- 直接操作 DOM，模擬使用者點擊分頁按鈕
...
readTotal()        → 正則從 body.textContent 取出總商品數
```
...
**總數正則**: `總計\s*(\d+)\s*項` | `共\s*(\d+)\s*筆` | `(\d+)\s*件\s*商品` | `架上商品\((\d+)\)`
