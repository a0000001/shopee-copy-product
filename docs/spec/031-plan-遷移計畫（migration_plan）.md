...
**保留的清單**（這些被 014-spec 引用，且與 popup「用 AI 更新 JSON」按鈕的商業邏輯相關）：

| 檔案 | 保留原因 |
|------|---------|
| `001-plan-商品上架流程（shopee_listing_pipeline）.md` | AI 按鈕需要知道上架管線順序 |
| `012-plan-商品圖片影片與描述規範（shopee_media_description）.md` | AI 按鈕需要知道圖片/影片格式與命名規則 |
| `013-guide-蝦皮商品描述爬取（shopee_scraping）.md` | AI 按鈕需要知道 WAF 限制與 API 可用性 |

> **待決定**：001/012/013 同時存在於原 repo（管線正本）與新 repo（extension AI 邏輯），可能 drift。解法選項：(a) 兩邊各加 cross-reference header 註明需同步更新；(b) 未來決定 SSOT 歸屬後另一邊改 stub。當前暫不處理。
...
| `所有非 001/011/012/013/014/015/016/021/099 的 spec` | 與 extension 無關 |

> **注意**：011（價格表→JSON 轉換）為一鍵開店系統核心，已遷移至新專案保留。
...
| 4 | 新專案 `docs/spec/` 包含 001、011、012、013、014、015、016、021、099 | 共 9 個檔案 |
...
### 4.3 Token 不要寫入任何 .md 或 .js 檔案

`.gitignore` 只保護 `.env`。若把 token 值寫進 markdown、程式碼或 commit message，GitHub secret scanning 會攔截 push（嚴重情況可能導致 token 被撤銷）。

**避免方式**：
- 永遠從 `.env` 讀取 token：`$token = (Get-Content .env | Select-String "GITHUB_TOKEN=") -replace ".*=", ""`
- 文件中的範例一律使用 `your_token_here` 佔位符

### 4.4 原 repo 可能有未 push 的變更
