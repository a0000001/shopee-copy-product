...
空白狀態由 popup.js 的 `showEmptyState()` 統一處理，依場景顯示不同引導，**不顯示冷冰冰的錯誤訊息**。

#### errorCode API contract

content script（`extractor.js`）回傳錯誤時，**必須同時附上 `errorCode` 欄位**，供 popup.js 做分支判斷。

> **判斷依據必須用 `errorCode`，禁止比對 `error` 文字內容**（文字可隨時調整，不可作為程式邏輯依據）。

| errorCode | 發生時機 | 在 `extractor.js` 的設置位置 |
|-----------|---------|---------------------------|
| `NOT_PRODUCT_PAGE` | URL 符合 shopee.tw 但不是商品詳情頁（如店鋪頁、搜尋頁、列表頁） | `isProductPage()` 返回 false 時 |

#### 各場景引導規則

| 場景 | 來源 | Icon | 標題 | 說明 | 去蝦皮按鈕 | 重新整理按鈕 |
|------|------|------|------|------|-----------|------------|
| 非蝦皮 URL | popup.js URL 前置檢查 | 🛒 | 目前頁面不是蝦皮商品頁 | 請前往蝦皮購物商品頁面 | ✅ | ❌ |
| 蝦皮店鋪/列表頁 | `errorCode === 'NOT_PRODUCT_PAGE'` | 🛒 | 目前頁面不是蝦皮商品頁 | 你目前在店鋪或列表頁，請點入任一商品詳情頁後再開啟外掛 | ✅ | ❌ |
| content script 無回應 | `sendMessage` 回應為空 | 🔄 | 內容腳本無回應 | 請嘗試重新整理頁面 | ❌ | ✅ |
| 其他讀取錯誤 | `resp.error` 有值但非已知 errorCode | ⚠️ | 無法讀取商品資料 | 顯示原始錯誤訊息 | ✅ | ✅ |
| 通訊例外 | `sendMessage` throw exception | ⚠️ | 通訊失敗 | 顯示例外訊息，請重新整理 | ❌ | ✅ |
